...
const DesignConfigurator = ({configId,imageUrl, imageDimension}: DesignConfiguratorProps) => {
    ...
    return (
        <div className=" relative mt-20 grid grid-cols-1 lg:grid-cols-3 mb-20 pb-20">
            <div className=" relative h-[37.5rem] overflow-hidden col-span-2 w-full max-w-4xl flex items-center justify-center rounded-lg border-2 border-dashed border-gray-300 p-12 text-center focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2" ref={containerRef}>
                                ...
                                <RadioGroup key={name} value={options[name]} onChange={(val)=>{
                                    ...
                                    <div className=" mt-3 space-y-4">
                                        {selectableOptions.map((option)=>(
                                            <RadioGroup.Option key={option.value} value={option} className={({active, checked})=>cn('relative block cursor-pointer rounded-lg bg-white px-6 py-6 shadow-sm border-2 border-zinc-200 focus:outline-none ring-0 focus:ring-0 outline-none sm:flex sm:justify-between',{
                                                'border-primary' : active|| checked
                                            })}>
                                                <span className=" flex items-center">
                                                    <span className=" flex flex-col text-sm">
                                                        <RadioGroup.Label as='span' className=' font-medium text-gray-900'> {option.Label}</RadioGroup.Label>
                                                        {option.description ?(
                                                ...
                                                <RadioGroup.Description as='span' className=" mt-2 text-sm flex sm:ml-4 sm:mt-0 sm:flex-col sm:text-right">
